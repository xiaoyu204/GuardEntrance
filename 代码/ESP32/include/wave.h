...
typedef struct WAV_HEADER_S {
    ...
    unsigned int formatSize{
        16}; // 4byte,fmt
             // chunk块的(compressionCode,numChannels,sampleRate,bytesPerSecond,blockAlign,bitsPerSample)大小，不包含自己
    ...
    unsigned int dataSize; // 4byte,从下个地址到文件结尾的总字节数。即除了wav header以外的pcm data length = 文件大小 -
                           // 44
} WAV_HEADER;

WAV_HEADER CreateWaveHeader(int            numChannels   = 1,           // 声道数
                            unsigned int   sampleRate    = SAMPLE_RATE, // 采样率
